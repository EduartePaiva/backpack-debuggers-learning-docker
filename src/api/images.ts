...
const ImageParams = z.object({
    ...
    size: z.coerce.number().min(50, "Minimum size is 50.").max(500, "Maximum size is 500."),
});
type ImageParams = z.infer<typeof ImageParams>;

router.get<ImageParams>("/monster/:dex_number/size/:size/image.png", async (req, res, next) => {
    try {
        ...
        const sourceImagePath = path.join(process.cwd(), "sprites", dex_number.toString() + ".png");
